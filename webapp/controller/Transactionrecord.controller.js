sap.ui.define([
	...
	"sap/ui/model/FilterOperator"
  	...
  	return Controller.extend("com.apptech.bfi-businessunit.controller.Transactionrecord", {
		...
		fgetAllTransaction: function (transtypefilter,oTransTatus) {
			...
			var urltag = "";
			if (value1 ===""){
				urltag ="https://xs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName="+ this.sDataBase +"&procName=spAppBusinessUnit&QUERYTAG=getFilteredTransactions&VALUE1=1&VALUE2=0&VALUE3=&VALUE4=";// getTransactions
			}else{
				urltag ="https://xs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName="+ this.sDataBase +"&procName=spAppBusinessUnit&QUERYTAG=getFilteredTransactions&VALUE1="+ value1 +"&VALUE2="+ value2 +"&VALUE3=&VALUE4=";
			}
			$.ajax({
				url: urltag,
				type: "GET",
		...
		fgetHeader: function (dbName, procName, queryTag, value1, value2, value3, value4) {
			...
			$.ajax({
				url: "https://xs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=" + dbName + "&procName=spAppBusinessUnit&QUERYTAG=" + queryTag + "&VALUE1=" + value1 +
		...
		fBuToBu: function (transtype,transno,oCardCode,oPostingDate,oMarkupType,oIssueBU,oReceiveBU,oRemarks,oDetails,oAttachment,oAttachmentKey) {
			...
			$.ajax({
				url: "https://18.141.110.57:50000/b1s/v1/InventoryGenExits",
				type: "POST",
		...
		fBuToCashSales: function (transtype,transno,oCardCode,oPostingDate,oMarkupType,oIssueBU,oReceiveBU,oRemarks,oDetails,oAttachment,oAttachmentKey) {
			...
			$.ajax({
			  ...
			  if (results) {
				...
				$.ajax({
				  url: "https://18.141.110.57:50000/b1s/v1/Invoices",
				  type: "POST",
				  ...
				  if (results) {
					////////
					  ...
					  //ajax call to SL
					  $.ajax({
						url: "https://18.141.110.57:50000/b1s/v1/IncomingPayments",
						type: "POST",
		...
		fBuToInterOrgReceipt: function (transtype,transno,oCardCode,oPostingDate,oMarkupType,oIssueBU,oReceiveBU,oRemarks,oDetails,oAttachment,oAttachmentKey) {
			...
			//ajax call to SL
			$.ajax({
				url: "https://18.141.110.57:50000/b1s/v1/InventoryGenEntries",
				type: "POST",
    ...
    /// Record Posting
		...
		fUpdatePending: function (transtype,transno,oCardCode,oPostingDate,oMarkupType,oIssueBU,oReceiveBU,oRemarks,ostatus,oDocType,oDetails,oAttachment,oAttachmentKey) {
			...
			$.ajax({
			url: "https://xs.biotechfarms.net/app-xsjs/ExecQuery.xsjs?dbName=" + this.sDataBase + "&procName=spAppBusinessUnit&queryTag=deleteDraftDetails&value1=" +
